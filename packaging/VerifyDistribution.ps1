...
try {
    ...
    Invoke-DotNet -Arguments @('pack', (Join-Path $repositoryRoot 'Icod.Host.csproj'), '-c', $Configuration, '--no-build', '--no-restore', '-o', $packageDirectory, '-p:ContinuousIntegrationBuild=true')
    & (Join-Path $PSScriptRoot 'VerifyPackageArtifact.ps1') -ArtifactDirectory $packageDirectory -Configuration $Configuration

...
function Invoke-Pack {
    ...
    Invoke-DotNet -Arguments @('pack', (Join-Path $repositoryRoot 'Icod.Host.csproj'), '-c', $Configuration, '--no-build', '--no-restore', '-o', $artifactDirectory)
}
function Invoke-Validate {
    Write-Host ''
    Write-Host "=== Validate ($Configuration) ==="
    & (Join-Path $PSScriptRoot 'VerifyPackageArtifact.ps1') -ArtifactDirectory $artifactDirectory -Configuration $Configuration
}

Push-Location $repositoryRoot
try {
    switch ($Section) {
        'all' { Invoke-Clean; Invoke-Restore; Invoke-Build; Invoke-Test; Invoke-Pack; Invoke-Validate }
        'clean' { Invoke-Clean }
